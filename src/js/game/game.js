...
class Game {
  ...
  updateUi(monster, player) {
    ...
    playerHealthProgressBar.setAttribute(
      'style',
      `background: linear-gradient(90deg, #860918 ${procentHealth}%, rgba(11,12,15,1) ${procentHealth}%);`,
    );
    ...
    monsterHealthProgressBar.setAttribute(
      'style',
      `background: linear-gradient(90deg, #860918 ${monsterProcentHealth}%, rgba(11,12,15,1) ${monsterProcentHealth}%);`,
    );
  ...
  updateGameStats(gameStats) {
    ...
    let allTimeGameBeated = document.getElementById('allTimeGameBeated');
    allTimeGameBeated.innerHTML = gameStats.allTime.beatedMonsters;
  }
